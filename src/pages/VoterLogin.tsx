...
const VoterLogin = () => {
  ...
  const handlePaste = (event: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    if (!VOTER_ID_PATTERN.test(pastedText)) {
      ...
      updateStatus('Use up to 64 characters: letters, numbers, hyphens, or slashes.', 'error');
      return;
    }
    event.preventDefault();
  ...
  const startWebcam = async () => {
    ...
    try {
      ...
      updateStatus('Camera active. Center your face, then verify your identity.', 'success');
    } catch (error) {
      console.error('Webcam error:', error);
      updateStatus('Camera access was denied or is unavailable. Check browser permissions and try again.', 'error');
    }
  };
  ...
  const verifyVoter = async (event?: React.FormEvent) => {
    ...
    if (!VOTER_ID_PATTERN.test(normalizedVoterId)) {
      updateStatus('Enter a valid Voter ID using letters, numbers, hyphens, or slashes.', 'error');
      return;
    }

    const video = videoRef.current;
    const canvas = canvasRef.current;
    if (!video || !canvas || !isCameraActive || video.readyState < 2 || !video.videoWidth) {
      updateStatus('Start the camera and wait for the live preview before verifying.', 'error');
      return;
    }

    const context = canvas.getContext('2d');
    if (!context) {
      updateStatus('The browser could not prepare the camera capture. Please try again.', 'error');
      return;
    }
    ...
    try {
      ...
      updateStatus(error instanceof Error ? error.message : 'Network error. Please check the authentication service.', 'error');
    } finally {
    ...
    <div className="grow flex flex-col items-center justify-center w-full">
      <div className="w-full max-w-md overflow-hidden rounded-md border border-hairline bg-paper shadow-sm">
        ...
        <form onSubmit={verifyVoter} className="p-6 sm:p-8 space-y-5">
          <div>
            <label className="text-sm font-medium text-ink" htmlFor="voterId">{t('voter.id_label')}</label>
            <input
              id="voterId"
              type="text"
              value={voterId}
            ...
            />
            <p id="voter-id-help" className="mt-1 text-xs text-muted-foreground">Letters, numbers, hyphens, and slashes only.</p>
          </div>
          ...
          <div>
            ...
            <div className="group relative aspect-video w-full overflow-hidden rounded-md border border-hairline bg-paper-warm">
              <video ref={videoRef} autoPlay playsInline muted className="h-full w-full scale-x-[-1] object-cover" aria-label="Live camera preview" />
              <canvas ref={canvasRef} className="hidden" />
              {!isCameraActive && (
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-paper-warm text-muted-foreground">
                  <svg className="w-12 h-12 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5v12z" /></svg>
          ...
          <div className="flex gap-3 pt-2">
            {!isCameraActive ? (
              <button type="button" onClick={startWebcam} className="inline-flex w-full items-center justify-center gap-2 rounded-md border border-hairline bg-paper px-4 py-2 text-sm font-semibold text-ink hover:border-ink">
                {t('voter.enable_cam')}
              </button>
            ) : (
              <button type="submit" disabled={isProcessing} className="inline-flex w-full items-center justify-center gap-2 rounded-md bg-saffron px-4 py-2 text-sm font-semibold text-paper shadow-sm hover:bg-saffron/90 disabled:cursor-not-allowed disabled:opacity-50">
