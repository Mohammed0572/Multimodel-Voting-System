...
function findTesseractBinary() {
  ...
  try {
    ...
    try {
      const whichCmd =
        os.platform() === "win32" ? "where tesseract" : "which tesseract";
      resolvedPath = execSync(whichCmd, {
        encoding: "utf-8",
        stdio: ["pipe", "pipe", "ignore"],
      })
        .trim()
  ...
  if (os.platform() === "win32") {
    ...
    try {
      ...
      const registryPaths = [
        ...registryOutput.matchAll(/InstallLocation\s+REG_SZ\s+(.+)/gi),
      ]
        .map((match) => match[1].trim())
        .filter(Boolean)
        .map((installLocation) => path.join(installLocation, "tesseract.exe"));
      commonWinPaths.push(...registryPaths);
    } catch {
      // Registry lookup is optional; continue with known paths.
    }
...
function installTesseract() {
  ...
  if (platform === "win32") {
    try {
      ...
      console.log(
        "[!] winget installation failed or unavailable. Trying choco...",
      );
    }
    ...
    try {
      ...
      console.error(
        `[X] Failed to install Tesseract via Homebrew: ${err.message}`,
      );
      process.exit(1);
    }
